...
router.post('/get', function(req, res) {
  var content = req.body;
	var gid = content.gid;
	if(gid){
		var sql = 'SELECT g.gid, g.gname, count(m.uid) mem_count from group_members m, groups g where g.gid = ' + gid + ' and m.gid = g.gid GROUP BY m.gid';
...
router.post('/joined', function(req, res) {
  var content = req.body;
	var uid = content.uid;
	if(uid){
		var sql = 'SELECT g.gid, g.gname from group_members m, groups g where m.uid = ' + uid + ' and m.gid = g.gid';
		mysql.query(sql, function(err, result){
...
router.post('/create', function(req, res) {
  var content = req.body;
	...
	if(uid && gname){
		//增加组
		var sql = 'insert into groups (gname, create_time, uid, remarks, state) value ("' + gname + '", CURRENT_TIMESTAMP, ' + uid + ', 0, 1)';
		mysql.query(sql, function(err, result){
			var gIdx = result.insertId;
			if(gIdx){
				
				//更新组成员信息
				var sql2 = 'insert into group_members (gid, uid, u_nickname, create_time) value (' + gIdx + ', ' + uid + ', "' + nickname + '", CURRENT_TIMESTAMP)';
...
router.post('/members', function(req, res) {
  var content = req.body;
	var gid = content.gid;
	if(gid){
		var sql = 'select * from groups where gid = ' + gid;
		mysql.query(sql, function(err, result){
			if(result){
				res.send(result);
			}else{
...
router.post('/msgList', function(req, res) {
  var content = req.body;
	...
	if(gid){
		...
		var sql = 'select uid from groups where gid = ' + gid + ' and uid = ' + uid;
		mysql.query(sql, function(err, result){
			if(result){
				
				//选取不同角色可读的信息列表
				if(result.length > 0){//是群主
					sql = 'select m.mid, m.type, m.content, u.uid, u.avatar_url, u.nickname from messages m, users u where m.gid = ' + gid + 
				...
				mysql.query(sql, function(err, result){
					if(result){
						...
						res.send('error getting group messages: ' + err);
					}
				});
				
				
				//更新最后一次阅读时间
				var sql2 = 'insert into group_last_read (gid, uid, last_time) value (' + gid + ', ' + uid + ', CURRENT_TIMESTAMP) on duplicate key update last_time = CURRENT_TIMESTAMP';
				mysql.query(sql2, function(err, result){ });
...
router.post('/addUserToGroup', function(req, res) {
  var content = req.body;
	...
	if(gid && uid){
		...
		//更新组成员信息
		var sql2 = 'insert into group_members (gid, uid, u_nickname, create_time) value (' + gid + ', ' + uid + ', "' + nickname + '", CURRENT_TIMESTAMP) on duplicate key update u_nickname = ' + nickname;
		mysql.query(sql2, function(err, result){
			if(result){
				res.send('200');
			}else{
...
router.post('/removeUsersFromGroup', function(req, res) {
  var content = req.body;
	...
	if(gid && uids.length > 0){
		...
		var sql = 'delete from group_members where gid = ' + gid + ' and (uid = ' + uid + ')';
		mysql.query(sql, function(err, result){
			if(result){
				res.send('200');
			}else{
...
router.post('/editGroupName', function(req, res) {
  var content = req.body;
	...
	if(gid){
		var sql = 'update groups set gname = ' + name + ' where gid = ' + gid;
		mysql.query(sql, function(err, result){
			if(result){
				res.send('200');
			}else{
...
router.post('/editNickname', function(req, res) {
  var content = req.body;
	...
	var nickname = content.nickname || '';
	if(gid){
		var sql = 'update group_members set u_nickname = ' + nickname + ' where gid = ' + gid + ' and uid = ' + uid;
		mysql.query(sql, function(err, result){
			if(result){
				res.send('200');
			}else{
...
router.post('/quitGroup', function(req, res) {
  var content = req.body;
	...
	if(gid){
		...
		mysql.query(sql, function(err, result){
			if(result && result.length <= 0){
				var sql2 = 'delete from group_members where gid = ' + gid + ' and uid = ' + uid;
